...
| 时间 | 事件 | 工作流 | 耗时 |
|------|------|--------|------|
| **07:00** | 🤖 自动生成日报 | `daily-update.yml` 触发 | - |
| **07:00-07:02** | 📝 运行 Python 脚本抓取新闻 | `Generate daily news` | ~2 分钟 |
| **07:02-07:03** | ⏳ 等待（确保文件完整） | `Wait for 2 minutes` | 1 分钟 |
| **07:03-07:05** | 📤 提交并推送代码 | `Commit and push` | ~2 分钟 |
| **07:05** | 🚀 触发自动部署 | `auto-deploy-all.yml` 触发 | - |
| **07:05-07:07** | ⏳ 等待（确保代码同步） | `Wait for 2 minutes` | 2 分钟 |
| **07:07-07:08** | 🌐 Netlify 部署 | `Deploy to Netlify` | ~1 分钟 |
| **07:08** | ✅ 网站可访问 + 钉钉通知 | - | - |
...
```
07:00 ──┬─────────────────────────────────────────────
        │ daily-update.yml 触发（Cron: 0 23 * * *）
        ↓
07:00 ──┼── [Checkout] 检出代码
        │
        ↓
07:00 ──┼── [Setup Python] 配置 Python 环境
        │
        ↓
07:00 ──┼── [Install] 安装依赖包
        │
...
07:00 ──┼── [Generate] 生成日报 ⭐
        ...
        │
        ↓
07:02 ──┼── [Wait 2min] 等待 2 分钟
        │   - 确保所有文件写入完成
        │
        ↓
07:03 ──┼── [Commit & Push] 提交并推送 ⭐
        │   - git add -A
...
07:05 ──┼── [Checkout] 检出代码
        │
        ↓
07:05 ──┼── [Wait 2min] 等待 2 分钟
        │   - 确保 GitHub 代码已同步
        │
        ↓
07:07 ──┼── [Setup Node] 配置 Node.js
        │
        ↓
07:07 ──┼── [Install Netlify] 安装 Netlify CLI
        │
        ↓
07:07 ──┼── [Deploy] Netlify 部署 ⭐
        │   - netlify deploy --prod --dir=.
        │
        ↓
07:08 ──┴── ✅ 部署完成，网站可访问 + 钉钉通知
```
